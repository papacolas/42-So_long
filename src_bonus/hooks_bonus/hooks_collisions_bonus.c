...
/*   Updated: 2025/04/08 15:18:09 by ncrivell         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_collision_with_enemies(t_vars *vars)
{
	...
	to_grid_next_x = vars->player_x / 32;
	to_grid_next_y = vars->player_y / 32;
	i = 0;
	while (i < vars->enemy_squad)
	{
		to_grid_bandit_x = vars->enemies[i].enemy_x / 32;
		to_grid_bandit_y = vars->enemies[i].enemy_y / 32;
		if (to_grid_next_x == to_grid_bandit_x
			&& to_grid_next_y == to_grid_bandit_y)
			return (1);
		i++;
	}
	return (0);
...
int	collision_with_collectible(t_vars *vars, int grid_x, int grid_y)
{
	...
	{
		vars->victory = 1;
		vars->speed_bandits = 1;
	}
	if (vars->victory == 1)
		change_map_before_victory(vars);
	return (0);
}
