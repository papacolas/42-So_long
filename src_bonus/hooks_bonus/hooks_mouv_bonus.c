...
/*   Updated: 2025/04/14 16:57:36 by ncrivell         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	altern_moves(t_vars *vars)
{
	if ((vars->moves_count % 2 == 0))
	{
		...
		{
			ft_putstr_fd("Error\nImpossible de charger image du joueur\n", 2);
			return (1);
		}
	}
	else
	{
		vars->textures.player.img_ptr = mlx_xpm_file_to_image(vars->mlx_ptr,
				"textures/playerf2.xpm", &vars->textures.player.width,
				&vars->textures.player.height);
		if (!vars->textures.player.img_ptr)
		{
			ft_putstr_fd("Error\nImpossible de charger image du joueur\n", 2);
...
int	make_mouvement(t_vars *vars, int keycode, int step)
{
	...
	if (!check_collision_with_wall(vars, save_x, save_y, step))
		return (0);
	if (check_collision_with_enemies(vars))
	{
...
int	mouvements_with_key(int keycode, t_vars *vars)
{
	...
	if (vars->end_game)
		return (0);
	if (vars->moves_count > 49 && (keycode == 97 || keycode == 100
