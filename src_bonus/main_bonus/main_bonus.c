...
/*   Updated: 2025/04/09 18:48:30 by ncrivell         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv)
{
	...
	if (initialize_game(&vars, argv[1]))
		return (1);
	if (load_textures(&vars) || init_enemies(&vars))
	{
		clean_before_exit(&vars);
		return (1);
	}
	render(&vars);
	mlx_loop_hook(vars.mlx_ptr, &generate_enemies_mov, &vars);
